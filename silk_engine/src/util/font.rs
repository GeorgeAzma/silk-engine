...
impl Font {
    ...
    pub fn gen_char_sdf(&self, char: char, size_px: u32) -> ImageData {
        ...
        impl Segment {
            ...
            fn sdf(&self, p: Vec2) -> f32 {
                if self.line {
                    ...
                    return pa.cross(ba).signum() * (ba * h.clamp(0.0, 1.0) - pa).len();
                }
                let ab = self.b - self.a;
                let d = self.c - 2.0 * self.b + self.a;
                let mut t = ((p - self.a).dot(ab) / ab.len2()).clamp(0.0, 1.0);
                // Newton iters
                for _ in 0..2 {
        ...
        for y in 0..h {
            for x in 0..w {
                let p = Vec2::new(x as f32, y as f32) / size_px as f32;
                let mut d = f32::MAX;
                let segs = grid.get(p);
                if segs.is_empty() {
                    continue;
                ...
                for &i in segs {
                    ...
                    // let c1 = (s.b - s.a).cross(p - s.a);
                    // let c2 = (s.c - s.b).cross(p - s.b);
                    // let c3 = (s.a - s.c).cross(p - s.c);
                ...
                let d = (d + E) / (2.0 * E);
                if d <= 1.0 {
                    sdf[(y * w + x) as usize] |= (d.clamp(0.0, 1.0) * 255.0) as u8;
                }
            }
        }
