use crate::RES_PATH;

pub struct ImageData {
    pub img: Vec<u8>,
    pub width: u32,
    pub height: u32,
    pub channels: u8,
}

struct Qoi;

const MAX_PIXELS: u32 = 400_000_000;
const SRGB: u8 = 0;
const LINEAR: u8 = 1;
const RGB_MASK: u8 = 0b1111_1110;
const RGBA_MASK: u8 = 0b1111_1111;
const INDEX_MASK: u8 = 0b0000_0000;
const DIFF_MASK: u8 = 0b0100_0000;
const LUMA_MASK: u8 = 0b1000_0000;
const RUN_MASK: u8 = 0b1100_0000;
const MIN_QOI_LEN: usize = 4 /* magic */ + 4 /* width */ + 4 /* height */ + 1 /* channels */ + 1 /* colorspace */ + 8 /* padding */;
impl Qoi {
    pub fn load(name: &str) -> ImageData {
        crate::scope_time!("QOI load");
        let path = format!("{RES_PATH}/images/{name}.qoi");
        let qoi = std::fs::read(path).unwrap_or_else(|_| panic!("qoi image not found: {name}"));
        assert_eq!(
            &qoi[0..4],
            b"qoif",
            "invalid qoi magic number: {}",
            std::str::from_utf8(&qoi[0..4])
                .unwrap_or_else(|_| panic!("invalid qoi magic number: {name}"))
        );
        let width = u32::from_be_bytes([qoi[4], qoi[5], qoi[6], qoi[7]]);
        assert_ne!(width, 0, "width is 0");
        let height = u32::from_be_bytes([qoi[8], qoi[9], qoi[10], qoi[11]]);
        assert_ne!(height, 0, "height is 0");
        let pixels = width * height;
        assert!(
            pixels <= MAX_PIXELS,
            "image too large: {pixels}px ({width}x{height}) > {}",
            MAX_PIXELS
        );
        let channels = qoi[12];
        assert!(
            channels == 3 || channels == 4,
            "invalid channel count: {channels}"
        );
        let img_len = (pixels * channels as u32) as usize;
        let mut img = Vec::with_capacity(img_len);
        unsafe {
            img.set_len(img_len);
        }
        let colorspace = qoi[13];
        assert!(
            colorspace == SRGB || colorspace == LINEAR,
            "invalid colorspace"
        );
        let mut i = 14;
        let mut px_idx = 0;
        if channels == 3 {
            let mut px: [u8; 3] = [0, 0, 0];
            let mut seen: [[u8; 3]; 64] = [[0, 0, 0]; 64];
            while px_idx < img_len {
                let mut run = 0;
                let tag = qoi[i];
                let rgb = match tag {
                    RGBA_MASK => {
                        panic!("RGBA chunk encountered in RGB image");
                    }
                    RGB_MASK => {
                        let rgb = [qoi[i + 1], qoi[i + 2], qoi[i + 3]];
                        i += 3;
                        rgb
                    }
                    tag => match tag & 0xC0 {
                        INDEX_MASK => seen[tag as usize],
                        DIFF_MASK => {
                            let dr = ((tag >> 4) & 0b11).wrapping_sub(2);
                            let dg = ((tag >> 2) & 0b11).wrapping_sub(2);
                            let db = (tag & 0b11).wrapping_sub(2);
                            let r = px[0].wrapping_add(dr);
                            let g = px[1].wrapping_add(dg);
                            let b = px[2].wrapping_add(db);
                            [r, g, b]
                        }
                        LUMA_MASK => {
                            i += 1;
                            let nxt = qoi[i];
                            let dg = (tag & 0x3F).wrapping_sub(32);
                            let dr = ((nxt >> 4) & 0x0F).wrapping_sub(8).wrapping_add(dg);
                            let db = (nxt & 0x0F).wrapping_sub(8).wrapping_add(dg);
                            let r = px[0].wrapping_add(dr);
                            let g = px[1].wrapping_add(dg);
                            let b = px[2].wrapping_add(db);
                            [r, g, b]
                        }
                        RUN_MASK => {
                            let run_len = tag & 0x3F;
                            assert!(run_len < 63, "illegal run-length: {} > 62", run_len);
                            run = run_len;
                            px
                        }
                        _ => panic!(),
                    },
                };
                for _ in 0..=run {
                    img[px_idx..px_idx + 3].copy_from_slice(&rgb[..3]);
                    px_idx += 3;
                }
                i += 1;
                seen[Self::hash3(rgb) as usize] = rgb;
                px = rgb;
            }
        } else if channels == 4 {
            let mut px: [u8; 4] = [0, 0, 0, 255];
            let mut seen: [[u8; 4]; 64] = [[0, 0, 0, 0]; 64];
            while px_idx < img_len {
                let mut run = 0;
                let tag = qoi[i];
                let rgba = match tag {
                    RGBA_MASK => {
                        let rgba = [qoi[i + 1], qoi[i + 2], qoi[i + 3], qoi[i + 4]];
                        i += 4;
                        rgba
                    }
                    RGB_MASK => {
                        let rgb = [qoi[i + 1], qoi[i + 2], qoi[i + 3], px[3]];
                        i += 3;
                        rgb
                    }
                    tag => match tag & 0xC0 {
                        INDEX_MASK => seen[tag as usize],
                        DIFF_MASK => {
                            let dr = ((tag >> 4) & 0b11).wrapping_sub(2);
                            let dg = ((tag >> 2) & 0b11).wrapping_sub(2);
                            let db = (tag & 0b11).wrapping_sub(2);
                            let [r, g, b, a] = px;
                            let r = r.wrapping_add(dr);
                            let g = g.wrapping_add(dg);
                            let b = b.wrapping_add(db);
                            [r, g, b, a]
                        }
                        LUMA_MASK => {
                            i += 1;
                            let nxt = qoi[i];
                            let dg = (tag & 0x3F).wrapping_sub(32);
                            let dr = ((nxt >> 4) & 0x0F).wrapping_sub(8).wrapping_add(dg);
                            let db = (nxt & 0x0F).wrapping_sub(8).wrapping_add(dg);
                            let [r, g, b, a] = px;
                            let r = r.wrapping_add(dr);
                            let g = g.wrapping_add(dg);
                            let b = b.wrapping_add(db);
                            [r, g, b, a]
                        }
                        RUN_MASK => {
                            let run_len = tag & 0x3F;
                            assert!(run_len < 63, "illegal run-length: {} > 62", run_len);
                            run = run_len;
                            px
                        }
                        _ => panic!(),
                    },
                };
                for _ in 0..=run {
                    img[px_idx..px_idx + 4].copy_from_slice(&rgba[..4]);
                    px_idx += 4;
                }
                i += 1;
                seen[Self::hash4(rgba) as usize] = rgba;
                px = rgba;
            }
        }
        assert_eq!(
            &qoi[i..i + 8],
            &[0, 0, 0, 0, 0, 0, 0, 1],
            "image does not end with padding"
        );
        ImageData {
            img,
            width,
            height,
            channels,
        }
    }

    pub fn save(name: &str, img: &[u8], width: u32, height: u32, channels: u8) -> usize {
        crate::scope_time!("QOI save");
        let pixels = width * height;
        assert!(
            pixels <= MAX_PIXELS,
            "image too large: {pixels}px ({width}x{height}) > {}",
            MAX_PIXELS
        );
        assert_eq!(
            img.len() as u32,
            pixels * channels as u32,
            "img size({}) doesn't match {width}x{height}x{channels}",
            img.len()
        );
        let mut qoi = Vec::with_capacity(pixels as usize * channels as usize);
        qoi.extend_from_slice(b"qoif");
        qoi.extend_from_slice(&width.to_be_bytes());
        qoi.extend_from_slice(&height.to_be_bytes());
        qoi.push(channels);
        let colorspace = SRGB;
        qoi.push(colorspace);
        let mut i = 0;
        if channels == 4 {
            let mut prev = [0, 0, 0, 255];
            let mut seen: [[u8; 4]; 64] = [[0, 0, 0, 0]; 64];
            while i < img.len() {
                let mut rgba = [img[i], img[i + 1], img[i + 2], img[i + 3]];
                if rgba == prev {
                    let mut run = 0;
                    while rgba == prev && run < 62 {
                        i += 4;
                        if i >= img.len() {
                            run += 1;
                            break;
                        }
                        prev = rgba;
                        rgba = [img[i], img[i + 1], img[i + 2], img[i + 3]];
                        run += 1;
                    }
                    run -= 1;
                    assert!(run < 62, "invalid run length: {run} > 61");
                    qoi.push(RUN_MASK | run);
                } else {
                    let hash = Self::hash4(rgba);
                    if seen[hash as usize] == rgba {
                        qoi.push(INDEX_MASK | hash);
                    } else {
                        if rgba[3] == prev[3] {
                            let dr = rgba[0].wrapping_sub(prev[0]).wrapping_add(2);
                            let dg = rgba[1].wrapping_sub(prev[1]).wrapping_add(2);
                            let db = rgba[2].wrapping_sub(prev[2]).wrapping_add(2);
                            if dr <= 3 && dg <= 3 && db <= 3 {
                                qoi.push(DIFF_MASK | dr << 4 | dg << 2 | db);
                            } else {
                                let dg = rgba[1].wrapping_sub(prev[1]).wrapping_add(32);
                                let dr = dr.wrapping_sub(dg).wrapping_add(8);
                                let db = db.wrapping_sub(dg).wrapping_add(8);
                                if dg <= 63 && dr <= 15 && db <= 15 {
                                    qoi.push(LUMA_MASK | dg);
                                    qoi.push((dr << 4) | db);
                                }
                            }
                        } else {
                            qoi.push(RGBA_MASK);
                            qoi.push(rgba[0]);
                            qoi.push(rgba[1]);
                            qoi.push(rgba[2]);
                            qoi.push(rgba[3]);
                        }
                        seen[hash as usize] = rgba;
                    }
                    i += 4;
                    prev = rgba;
                }
            }
        } else if channels == 3 {
            let mut prev = [0, 0, 0];
            let mut seen: [[u8; 3]; 64] = [[0, 0, 0]; 64];
            while i < img.len() {
                let mut rgb = [img[i], img[i + 1], img[i + 2]];
                if rgb == prev {
                    let mut run = 0;
                    while rgb == prev && run < 62 {
                        i += 3;
                        if i >= img.len() {
                            run += 1;
                            break;
                        }
                        prev = rgb;
                        rgb = [img[i], img[i + 1], img[i + 2]];
                        run += 1;
                    }
                    run -= 1;
                    assert!(run < 62, "invalid run length: {run} > 61");
                    qoi.push(RUN_MASK | run);
                } else {
                    let hash = Self::hash3(rgb);
                    if seen[hash as usize] == rgb {
                        qoi.push(INDEX_MASK | hash);
                    } else {
                        let dr = rgb[0].wrapping_sub(prev[0]).wrapping_add(2);
                        let dg = rgb[1].wrapping_sub(prev[1]).wrapping_add(2);
                        let db = rgb[2].wrapping_sub(prev[2]).wrapping_add(2);
                        if dr <= 3 && dg <= 3 && db <= 3 {
                            qoi.push(DIFF_MASK | (dr << 4) | (dg << 2) | db);
                        } else {
                            let dgu = rgb[1].wrapping_sub(prev[1]).wrapping_add(32);
                            let dru = dr.wrapping_sub(dg).wrapping_add(8);
                            let dbu = db.wrapping_sub(dg).wrapping_add(8);
                            if dgu <= 63 && dru <= 15 && dbu <= 15 {
                                qoi.push(LUMA_MASK | dgu);
                                qoi.push((dru << 4) | dbu);
                            } else {
                                qoi.push(RGB_MASK);
                                qoi.push(rgb[0]);
                                qoi.push(rgb[1]);
                                qoi.push(rgb[2]);
                            }
                        }

                        seen[hash as usize] = rgb;
                    }
                    i += 3;
                    prev = rgb;
                }
            }
        }
        qoi.extend_from_slice(&[0, 0, 0, 0, 0, 0, 0, 1]);
        let qoi_len = qoi.len();
        assert!(qoi_len >= MIN_QOI_LEN, "qoi too small");
        let img_path = format!("{RES_PATH}/images/{name}.qoi");
        std::fs::write(&img_path, &qoi)
            .unwrap_or_else(|e| panic!("failed to save qoi image({}): {e}", img_path));
        qoi_len
    }

    #[inline(always)]
    fn hash4(rgba: [u8; 4]) -> u8 {
        let v = u32::from_le_bytes(rgba) as u64;
        let s = ((v & 0xFF00_FF00) << 32) | (v & 0x00FF_00FF);
        (s.wrapping_mul(0x0300_0700_0005_000B_u64) >> 56) as u8 & 63
    }

    #[inline(always)]
    fn hash3(rgb: [u8; 3]) -> u8 {
        let v = u32::from_le_bytes([rgb[0], rgb[1], rgb[2], 0]) as u64;
        let s = ((v & 0xFF00_FF00) << 32) | (v & 0x00FF_0000);
        (s.wrapping_mul(0x0300_0700_0005_0000_u64) >> 56) as u8 & 63
    }
}

#[cfg(test)]
mod test {
    use super::*;

    fn assert_img_eq(a: &[u8], b: &[u8], width: u32, height: u32, channels: u8) {
        assert_eq!(a.len(), b.len(), "img size not equal");
        if a != b {
            println!("left img (wrong):");
            crate::print_img(a, width, height, channels);
            println!("right img (truth):");
            crate::print_img(b, width, height, channels);
            panic!();
        }
    }

    fn save_load(img: &[u8], width: u32, height: u32, channels: u8) {
        Qoi::save("temp", &img, width, height, channels);
        let loaded_img = Qoi::load("temp");
        assert_eq!(width, loaded_img.width);
        assert_eq!(height, loaded_img.height);
        assert_eq!(channels, loaded_img.channels);
        assert_img_eq(&loaded_img.img, &img, width, height, channels);
    }

    #[test]
    fn save_load_test() {
        *crate::INIT_IMG_PATH;
        let img = vec![
            155, 000, 000, /**/ 155, 000, 000, // run len + rgb
            000, 200, 000, /**/ 250, 168, 250, // luma
            000, 000, 155, /**/ 001, 000, 153, // diff
            000, 200, 000, /**/ 250, 168, 250, // index
        ];
        save_load(&img, 2, 4, 3);

        save_load(&b"AAAAAA".repeat(64), 2 * 64, 1, 3); // big run len
        save_load(&b"AAAEEE".repeat(64), 2 * 64, 1, 3); // big luma
        save_load(&b"AAABBB".repeat(64), 2 * 64, 1, 3); // big diff
        save_load(&b"QQQAAA".repeat(64), 2 * 64, 1, 3); // big index
        save_load(
            &[b"AAAAAA".repeat(64), b"EEE".repeat(63)].concat(),
            2 * 64 + 63,
            1,
            3,
        ); // big double run len

        // visualize correctly loaded/saved (hopefuly) QOI image
        let img = Qoi::load("../../../res/images/cursor");
        crate::print_img(&img.img, img.width, img.height, img.channels);
        Qoi::save("temp", &img.img, img.width, img.height, img.channels);
        let img = Qoi::load("temp");
        crate::print_img(&img.img, img.width, img.height, img.channels);
        panic!();

        for _ in 0..128 {
            let img = Qoi::load("../../../../../../Users/lumim/Downloads/backpack");
            let len = Qoi::save("temp", &img.img, img.width, img.height, img.channels);
            crate::log!("Compression: x{}", img.img.len() as f32 / len as f32);
        }
    }
}
